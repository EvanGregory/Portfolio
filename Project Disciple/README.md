...
Importantly, we also wanted these unlocks to be easily tunable by designers, so they wouldn't need to go change the code if they wanted something to unlock based on a completely different thing happening. This meant that just hard coding a bunch of places which call functions on specific other bits of code was out of the question. I ended up building out our unlock system and quest system to accomplish these goals. 
